...
class SortableTest extends Component
{
    ...
    {
        \Log::info('SortableTest: Updating order', ['orderedIds' => $orderedIds]);
        ...
        \Log::info('SortableTest: New order', ['items' => collect($this->items)->pluck('id')->toArray()]);

        // Dispatch event for debugging
        $this->dispatch('test-reordered', ['orderedIds' => $orderedIds]);

        // Show success message
        session()->flash('message', 'Items reordered successfully! New order: ' . implode(', ', $orderedIds));
    }
    ...
    public function resetOrder()
    {
        ...
        session()->flash('message', 'Order reset to default!');
    }
